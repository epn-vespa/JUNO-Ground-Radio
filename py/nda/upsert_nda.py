...
def nda_metadata(file_cdf, file_pdf, file_type):
    ...
    md['time_min'] = datetime.datetime(cdf.attrs['PDS_Observation_start_time'][0])
    md['time_max'] = datetime.datetime(cdf.attrs['PDS_Observation_stop_time'][0])
...
def ingest(md, db_cur, debug=False):
    """
    ...
    for item in md['tab']:
        ...
        query = "select count(*) from nda.metadata where granule_uid = {}".format(item['granule_uid'])
        ...
        if number_of_rows == 1:
            if debug:
                print "Record found, preparing update..."
            ...
            if debug:
                print "Record not found, preparing insert..."
...
if sys.argv[1] == '--help':
    ...
    print "SYNOPSIS"
    print "    upsert_nda.py --help"
    print "    upsert_nda.py [--verb] cdf_file pdf_file"
    print "DESCRIPTION"
    ...
    if debug:
        ...
        fcdf = sys.argv[2]
        fpdf = sys.argv[3]
        print "CDF file: {}".format(fcdf)
        print "PDF file: {}".format(fpdf)
...
# print metadata
    ...
    if debug:
        print "Connected to database {} at {} with user {}".format(mydatabase, myhost, myuser)

    ingest(metadata, cur, debug)
