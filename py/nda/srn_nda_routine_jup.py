...
def get_paths(cdf_version):
	root_dir = '/Users/baptiste/Projets/EUROPLANET/H2020/VESPA-VAA/Nancay/NDA/Routine_jupiter/'
	master_path = root_dir+'svn/trunk/cdf/master/'
	cdfbin_path = '/Applications/cdf/cdf36_0-dist/bin/'
	cdfout_path = root_dir+"data/cdf/v{}/".format(cdf_version)
...
def load_rt1_data(file_rt1):
	...
	dt_raw = np.dtype([('ss','a',405),('bb',[('rec_hr','b'),('rec_min','b'),('rec_sec','b'),('rec_cs','b'),('data','b',400),('status','b')],nspectra)])
	...
	for i in np.arange(nspectra):
		...
		if ts_error == 1:
			...
			print 'Error context:'
			if i != 0:
				print '- Timestamp at record {} : ({:02d}:{:02d}:{:02d}.{:02d})'.format(i-1,int(bb['rec_hr'][i-1]),int(bb['rec_min'][i-1]),int(bb['rec_sec'][i-1]),int(bb['rec_cs'][i-1]))
			print '- Timestamp at record {} : ({:02d}:{:02d}:{:02d}.{:02d})'.format(i,int(bb['rec_hr'][i]),int(bb['rec_min'][i]),int(bb['rec_sec'][i]),int(bb['rec_cs'][i]))
			if i != nspectra-1:
				print '- Timestamp at record {} : ({:02d}:{:02d}:{:02d}.{:02d})'.format(i+1,int(bb['rec_hr'][i+1]),int(bb['rec_min'][i+1]),int(bb['rec_sec'][i+1]),int(bb['rec_cs'][i+1]))
			#temporary fix:
			if int(bb['rec_cs'][i]) == 100:
				bb['rec_cs'][i] = 0
				bb['rec_sec'][i] += 1
				print 'Fixed Centiseconds value...'
	...
	if header_version == 1:
		...
		header['meridian_datetime'] = datetime.datetime(int(raw_header['merid_yr']),int(raw_header['merid_mo']),int(raw_header['merid_dd']),0,0,0)
	else:
		header['meridian_datetime'] = datetime.datetime(int(raw_header['merid_yr']),int(raw_header['merid_mo']),int(raw_header['merid_dd']),int(raw_header['merid_hh']),int(raw_header['merid_mm']),0)
	...
	header['start_datetime'] = datetime.datetime(int(raw_header['merid_yr']),int(raw_header['merid_mo']),int(raw_header['merid_dd']),raw_header['beg_hh'],raw_header['beg_mm'],raw_header['beg_ss'],raw_header['beg_ms']*1000)
	header['stop_datetime']  = datetime.datetime(int(raw_header['merid_yr']),int(raw_header['merid_mo']),int(raw_header['merid_dd']),raw_header['end_hh'],raw_header['end_mm'],raw_header['end_ss'],raw_header['end_ms']*1000)+datetime.timedelta(milliseconds=header['swp_time'])
	...
	if header_version == 6:
		if int(raw_header['h_stp_hh']) != header['stop__time'].hour:
			print 'warning, Header Stop time (HH) inconsistent with data records! Keeping data record value. Header = {:02d}:{:02d} / Data = {:02d}:{:02d}'.format(int(raw_header['h_stp_hh']),int(raw_header['h_stp_mm']),header['stop__time'].hour,header['stop__time'].minute)
		if int(raw_header['h_stp_mm']) != header['stop__time'].minute:
			print 'warning, Header Stop time (MM) inconsistent with data records! Keeping data record value. Header = {:02d}:{:02d} / Data = {:02d}:{:02d}'.format(int(raw_header['h_stp_hh']),int(raw_header['h_stp_mm']),header['stop__time'].hour,header['stop__time'].minute)
	...
	if header_version < 5:
		...
		if (header['rf_filt1'] == 0): 
			...
			header['rf_filt1_time'] = datetime.datetime.combine(header['start_datetime'].date(),datetime.time(int(raw_header['rf1_hour']),int(raw_header['rf1_minu'])))
		...
		if (header['rf_filt2'] == 0): 
			...
			header['rf_filt2_time'] = datetime.datetime.combine(header['start_datetime'].date(),datetime.time(int(raw_header['rf2_hour']),int(raw_header['rf2_minu'])))
	...
	var = {}
	var['time'] = np.array([datetime.datetime.combine(header['start_datetime'].date(),datetime.time(int(bb['rec_hr'][i]),int(bb['rec_min'][i]),int(bb['rec_sec'][i]),int(bb['rec_cs'][i])*10000)) for i in np.arange(nspectra)]).tolist()
...
def build_edr_cdf(file,build_cdf_master):
	
	cdf_version = '12'
	sft_version = '12'
	dat_version = '10'
	
	master_path, cdfbin_path, cdfout_path = get_paths(cdf_version)
	
	edr = build_edr_data(file)
	
	master_cdf = master_cdf_name(edr['header']['obsty_id'], edr['header']['instr_id'], edr['header']['recvr_id'], 'edr', cdf_version)
	skelet_cdf = master_skt_name(edr['header']['obsty_id'], edr['header']['instr_id'], edr['header']['recvr_id'], 'edr', cdf_version)
	...
	cdfout_file = "{}_{}_{}_edr_{:%Y%m%d%H%M}_{:%Y%m%d%H%M}_V{}.cdf".format(edr['header']['obsty_id'],edr['header']['instr_id'],edr['header']['recvr_id'], edr['time'][0], edr['time'][ndata-1],cdf_version)
...
# 	cdfout.attrs['NDA_calibration_attenuation'] = tmp_cal_atten
	...
	cdfout.attrs['Parents'] = os.path.basename(file)
